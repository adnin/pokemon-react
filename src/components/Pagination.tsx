...
export default function Pagination({result, params, updateParams, updatePage}: IPagination) {
  ...
  return (
    <div className='flex flex-row item-center py-12 space-x-4 justify-center'>
        <Button disabled={!result.previous} text="Previous" onClick={() => updatePage('Previous')} />
        <div className='flex flex-row items-center px-4 py-2 bg-transparent '>
            <span className='text-gray-400 text-lg'>Show</span>
            <input type="text" value={params.limit} pattern="[0-9]*" onInput={handleChange} className='text-center text-xl w-8 text-white bg-transparent mx-1 font-bold border-b focus:outline-none' /> 
            <span className='text-gray-400 text-lg'>results</span>
        </div>
        <Button disabled={!result.next} text="Next" onClick={() => updatePage('Next')} />
    </div>
  )
}
